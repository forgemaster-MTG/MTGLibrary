...
const CreateDeckPage = () => {
    ...
    const handleBasicsSubmit = (selectedFormat, isMockupActive) => {
        // e.preventDefault() removed as we call it directly with string
        if (selectedFormat === 'Commander' || selectedFormat === 'Standard') {
            fetchCommanders(selectedFormat, isMockupActive);
        } else {
            // Unused fallback for now
            console.warn("Unknown format selected");
        }
    };
    ...
    const handleCommanderSelect = (card) => {
        ...
        if (format === 'Standard') {
            ...
            // Just go to Review directly as AI is disabled for Standard for now
            addToast("Standard AI Deck Tech coming soon! Please verify on Discord to prioritize.", "info");
    ...
    const createDeck = async () => {
        ...
        try {
            const payload = {
                name: name || (selectedPartner ? `${selectedCommander.name} & ${selectedPartner.name}` : selectedCommander?.name) || 'New Commander Deck',
                format,
    ...
    const PosterCard = ({ card, onClick, isSelected }) => {
        ...
        return (
            <div
                ...
                className={`group relative h-[480px] rounded-3xl cursor-pointer transition-all duration-300 hover:-translate-y-2 ${isSelected ? 'ring-4 ring-indigo-500 scale-105' : ''}`}
            >
                {/* Glass Container */}
                <div className="absolute inset-0 bg-gray-900/40 backdrop-blur-md rounded-3xl border border-white/5 shadow-2xl group-hover:border-indigo-500/50 group-hover:shadow-indigo-500/20 transition-all overflow-hidden flex flex-col">
                    ...
                    <div className="h-[45%] p-5 flex flex-col relative">
                        ...
                        <div className="mt-4 flex-1 flex flex-col gap-2">
                            <div>
                                <h3 className="text-lg font-bold text-white leading-tight line-clamp-2 mb-1 group-hover:text-indigo-300 transition-colors">
                                    {card.name}
                                </h3>
                        ...
                        <div className="flex justify-between items-center mt-3 pt-3 border-t border-white/5">
                            ...
                            <span className="text-[10px] uppercase font-bold tracking-widest text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity">Select</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        const renderGroupHeader = (group, isExpanded, toggle) => (
            ...
            >
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-white group-hover:text-indigo-300 transition-colors">{group.identity.badge}</h3>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="bg-gray-800/50 rounded-xl p-1 flex border border-white/10">
                            <button
                                ...
                                className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                                title="List View"
                            >
                            ...
                            <button
                                ...
                                className={`p-2 rounded-lg transition-all ${viewMode === 'folder' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                                title="Folder View"
                            >
                        ...
                        <>
                            {sortedGroups.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
                                    ...
                                    <button
                                        onClick={() => setIsSearchOpen(true)}
                                        className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-indigo-500/20 hover:-translate-y-1 transition-all flex items-center gap-2"
                                    >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {sortedGroups.map(([key, group]) => (
                                        ...
                                        >
                                            ...
                                            <div>
                                                <h3 className="text-2xl font-black text-white group-hover:text-indigo-300 transition-colors uppercase tracking-wide">{group.identity.badge}</h3>
                                                <div className="h-0.5 w-8 bg-indigo-500/50 mx-auto my-2" />
                    ...
                    {viewMode === 'folder' && activeFolder && (
                        <div className="space-y-6">
                            <button
                                ...
                                className="flex items-center gap-2 text-indigo-400 hover:text-white transition-colors font-bold uppercase text-xs tracking-widest mb-4"
                            >
                        ...
                        <div className="space-y-4">
                            {sortedGroups.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in border border-dashed border-gray-800 rounded-3xl">
                                    ...
                                    <button
                                        onClick={() => setIsSearchOpen(true)}
                                        className="text-indigo-400 font-bold hover:text-indigo-300 mt-2 hover:underline"
                                    >
    ...
    const renderAIStrategy = () => (
        <div className="flex flex-col items-center justify-center h-[50vh] animate-fade-in text-center">
            <div className="relative mb-8">
                ...
                <div className="w-24 h-24 bg-indigo-600 rounded-full flex items-center justify-center relative shadow-[0_0_50px_rgba(79,70,229,0.5)]">
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div className="flex gap-2">
                        {[STEPS.BASICS, STEPS.COMMANDER, STEPS.AI_STRATEGY, STEPS.REVIEW].map((s, i) => (
                            <div key={s} className={`h-1.5 rounded-full transition-all duration-500 ${step >= s ? 'w-12 bg-indigo-500' : 'w-4 bg-gray-700'}`} />
                        ))}
                    </div>
                </div>
                            ...
                            <div className="w-full max-w-5xl px-4 space-y-12">
                                    ...
                                    <div className="bg-gray-800/50 rounded-2xl p-6 border border-white/5 backdrop-blur-sm animate-fade-in-down">
                                        ...
                                        <div className="flex gap-6">
                                            <div className="relative group">
                                                <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-2xl opacity-50 group-hover:opacity-75 blur transition duration-500" />
                                                <img
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <h2 className="text-xl font-black text-white uppercase tracking-[0.3em]">Commander</h2>
                                        <div className="h-px flex-1 bg-gradient-to-r from-transparent via-white/20 to-transparent" />
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div
                                            onClick={() => { setIsMockup(false); setFormat('Commander'); handleBasicsSubmit('Commander', false) }}
                                            className="group cursor-pointer bg-gray-800/40 hover:bg-indigo-900/20 border border-white/10 hover:border-indigo-500/50 rounded-3xl p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-indigo-500/10 flex flex-col items-center text-center gap-4 relative overflow-hidden"
                                        >
                                            <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                                            <div className="w-20 h-20 bg-indigo-500/20 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-500 border border-white/5 group-hover:border-indigo-500/30">
                                                <svg className="w-10 h-10 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
                                            </div>
                                            <div className="relative z-10">
                                                <h3 className="text-2xl font-black text-white mb-2 group-hover:text-indigo-300 transition-colors">From Collection</h3>
                                        ...
                                        {/* Commander Mockup */}
                                        <div
                                            onClick={() => { setIsMockup(true); setFormat('Commander'); handleBasicsSubmit('Commander', true) }}
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div
                                            ...
                                            className="group cursor-pointer bg-gray-800/40 hover:bg-emerald-900/20 border border-white/10 hover:border-emerald-500/50 rounded-3xl p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-emerald-500/10 flex flex-col items-center text-center gap-4 relative overflow-hidden"
                                        >
                                            <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                                            <div className="w-20 h-20 bg-emerald-500/20 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-500 border border-white/5 group-hover:border-emerald-500/30">
                                                <svg className="w-10 h-10 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
                                            </div>
                                            <div className="relative z-10">
                                                <h3 className="text-2xl font-black text-white mb-2 group-hover:text-emerald-300 transition-colors">Standard Collection</h3>
                                                <p className="text-sm text-gray-400 leading-relaxed">Build a 60-card Standard deck.<br />Using your own cards.</p>
                                            </div>
                                        </div>

                                        {/* Standard Mockup */}
                                        <div
                                            onClick={() => { setIsMockup(true); setFormat('Standard'); handleBasicsSubmit('Standard', true) }}
                                            className="group cursor-pointer bg-gray-800/40 hover:bg-cyan-900/20 border border-white/10 hover:border-cyan-500/50 rounded-3xl p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-cyan-500/10 flex flex-col items-center text-center gap-4 relative overflow-hidden"
                                        >
                                            <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                                            <div className="w-20 h-20 bg-cyan-500/20 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-500 border border-white/5 group-hover:border-cyan-500/30">
                                            ...
                                            <div className="relative z-10">
                                                <h3 className="text-2xl font-black text-white mb-2 group-hover:text-cyan-300 transition-colors">Standard Theorycraft</h3>
                                                <p className="text-sm text-gray-400 leading-relaxed">Test new Standard meta decks.<br />Without owning every card.</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
