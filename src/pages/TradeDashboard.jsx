            ...
            <div className="grid gap-4">
                {list.map(trade => (
                    <div
                        ...
                        className="bg-gray-800 border border-gray-700 hover:border-primary-500 p-4 rounded-xl cursor-pointer transition-all flex justify-between items-center"
                    >
                        <div>
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in text-gray-200">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-white flex items-center gap-3">
                        <span className="p-2 bg-primary-500/10 rounded-xl">
                ...
                <div className="flex gap-3">
                    <button
                        ...
                        className="bg-gray-800 hover:bg-gray-700 border border-gray-700 text-primary-400 px-4 py-2 rounded-lg font-bold flex items-center gap-2"
                    >
                    ...
                    <button
                        ...
                        className="bg-primary-600 hover:bg-primary-500 text-white px-5 py-2 rounded-lg font-bold shadow-lg shadow-primary-500/20 flex items-center gap-2"
                    >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                        New Trade
                    </button>
            ...
            <div className="flex border-b border-gray-700 mb-6">
                <button onClick={() => setActiveTab('active')} className={`pb-3 px-6 text-sm font-bold border-b-2 transition-colors ${activeTab === 'active' ? 'border-primary-500 text-white' : 'border-transparent text-gray-400 hover:text-white'}`}>
                    Active Trades {activeTrades.length > 0 && <span className="ml-2 px-2 py-0.5 bg-primary-600 text-white text-[10px] rounded-full">{activeTrades.length}</span>}
                </button>
                <button onClick={() => setActiveTab('history')} className={`pb-3 px-6 text-sm font-bold border-b-2 transition-colors ${activeTab === 'history' ? 'border-primary-500 text-white' : 'border-transparent text-gray-400 hover:text-white'}`}>
                    History
                </button>
                <button onClick={() => setActiveTab('armory')} className={`pb-3 px-6 text-sm font-bold border-b-2 transition-colors ${activeTab === 'armory' ? 'border-primary-500 text-white' : 'border-transparent text-gray-400 hover:text-white'}`}>
                    The Armory (Matches)
                </button>
            </div>
                    ...
                    {activeTab === 'armory' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {matches.length === 0 ? (
                                ...
                                <div key={`${match.cardId}-${idx}`} className="bg-gray-800 hover:bg-gray-750 border border-gray-700 hover:border-primary-500/50 rounded-xl p-4 flex gap-4 transition-all group relative overflow-hidden">
                                    {/* Background accent */}
                                    <div className="absolute top-0 right-0 w-24 h-24 bg-primary-500/5 rounded-full blur-2xl -mr-10 -mt-10 pointer-events-none"></div>
                                    ...
                                    <div className="flex flex-col justify-between flex-1 min-w-0 z-10">
                                        <div>
                                            ...
                                            <div className="text-xs text-primary-400 font-mono mt-1">{match.set?.toUpperCase()} • {match.finish}</div>

                                            <div className="mt-3 flex items-center gap-2">
                                                <div className="text-xs text-gray-400">Available from:</div>
                                                <Link
                                                    to={`/profile/${match.friendId}`}
                                                    className="px-2 py-0.5 bg-primary-900/30 text-primary-300 rounded text-xs font-bold border border-primary-500/20 hover:bg-primary-500 hover:text-white transition-colors cursor-pointer"
                                                >
                                                    {match.friendName}
                                                </Link>
                                            </div>
                                        ...
                                        <button
                                            ...
                                            className="mt-3 w-full py-2 bg-gray-700 hover:bg-primary-600 text-white text-xs font-bold uppercase rounded-lg transition-colors flex items-center justify-center gap-2"
                                        >
