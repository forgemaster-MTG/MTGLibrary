    ...
    if (isXS) {
        return (
            <div
                ...
                className="bg-indigo-500/10 border border-indigo-500/20 rounded-3xl h-full flex flex-col items-center justify-center cursor-pointer hover:bg-indigo-500/20 transition-all group"
            >
                <div className="w-3 h-3 bg-indigo-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(99,102,241,0.5)] group-hover:scale-125 transition-transform" />
                <span className="text-[10px] font-black text-indigo-400 mt-2 uppercase tracking-widest">Support</span>
            </div>
        );
    }
                        ...
                        {tickets.slice(0, isLargePlus ? 4 : 2).map((ticket) => (
                            ...
                            >
                                <div className="flex justify-between items-start gap-2">
                                    <span className="text-xs text-white font-medium truncate group-hover:text-indigo-400 transition-colors">{ticket.title}</span>
