...
const CommunityWidget = ({ size }) => {
    ...
    if (isXS) {
        return (
            <div onClick={handleManage} className="bg-primary-900/10 border border-primary-500/20 rounded-3xl h-full flex items-center justify-between px-4 cursor-pointer hover:bg-primary-500/20 transition-all group overflow-hidden">
                ...
                <div className="relative flex-shrink-0">
                    <svg className="w-4 h-4 text-primary-400 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
                    {pendingCount > 0 && (
                        <span className="absolute -top-1 -right-1 w-2 h-2 bg-primary-500 rounded-full border border-gray-950 animate-pulse" />
                    )}
                </div>
            </div>
        );
    ...
    return (
        <div className={`bg-gray-900/50 rounded-3xl ${isSmall ? 'p-4' : 'p-6'} border border-gray-800 hover:border-primary-500/30 transition-all group relative overflow-hidden flex flex-col h-full`}>
            <div className="absolute top-0 right-0 w-32 h-32 bg-primary-500/10 rounded-full blur-2xl -mr-16 -mt-16 group-hover:bg-primary-500/20 transition-all pointer-events-none" />

            <div className="flex justify-between items-start mb-4 relative z-10">
                <div>
                    <h3 className="text-gray-400 text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                        My Pod
                        {pendingCount > 0 && <span className="bg-primary-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full animate-pulse tracking-normal">{pendingCount} New</span>}
                    </h3>
                ...
                <div className="p-2 bg-primary-500/10 rounded-xl text-primary-400 group-hover:scale-110 transition-transform">
                        ...
                        <div className="flex-grow flex flex-col justify-center gap-4">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full bg-gray-950/80 border border-white/5 rounded-2xl p-4 pr-12 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-primary-500/50 transition-all shadow-inner"
                                />
                                <button className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-primary-600 rounded-xl text-white hover:bg-primary-500 transition-colors">
                        ...
                        <div className="flex gap-4 items-center">
                            {friends.slice(0, 5).map((rel, i) => (
                                <div key={rel.id} className="flex flex-col items-center gap-2 group/u cursor-pointer">
                                    <div className="relative">
                                        <div className="w-12 h-12 rounded-2xl bg-primary-900 border border-primary-500/30 flex items-center justify-center text-lg font-black text-primary-200 uppercase group-hover/u:scale-110 group-hover/u:border-primary-400 transition-all shadow-lg overflow-hidden">
                                            {rel.friend.username?.[0]}
                                            <div className="absolute inset-0 bg-gradient-to-t from-primary-950/50 to-transparent opacity-0 group-hover/u:opacity-100 transition-opacity" />
                                        </div>
                    ...
                    <div className="space-y-3">
                        ...
                        {[
                            ...
                            <div key={i} className="flex items-center gap-3 p-2 bg-gray-950/50 rounded-xl border border-white/5 text-[11px] group/item hover:bg-gray-800 transition-colors">
                                <div className="w-7 h-7 rounded-lg bg-primary-900 flex items-center justify-center font-bold text-primary-300 text-[10px] uppercase shadow-sm">{a.user[0]}</div>
                    ...
                    <div className="space-y-2">
                        {friends.slice(0, isMedium ? 3 : 2).map((rel) => (
                            <div key={rel.id} className="flex items-center gap-3 p-2 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer">
                                <div className="w-7 h-7 rounded-lg bg-primary-900 flex items-center justify-center text-[10px] font-bold text-primary-200 uppercase">{rel.friend.username?.[0]}</div>
            ...
            {!isXL && (
                <button
                    ...
                    className={`w-full ${isSmall ? 'py-2 text-[10px]' : 'py-3 text-xs'} bg-primary-600 hover:bg-primary-500 text-white font-black uppercase tracking-widest rounded-xl shadow-lg transition-all mt-4`}
                >
                    Manage Pod
                </button>
            )}
