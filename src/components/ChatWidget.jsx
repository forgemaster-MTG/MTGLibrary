...
const ChatWidget = () => {
    ...
    const [messages, setMessages] = useState([
        { role: 'model', content: '<p class="mb-2 text-gray-300">Hello! I am <strong>MTG Forge</strong>. How can I help you today? ⚔️</p>' }
    ]);
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            const history = messages.map(m => ({ role: m.role, content: m.content })); // Filter if needed
            const responseHtml = await GeminiService.sendMessage(apiKey, history, userMsg);
    ...
    return (
        <div className={`fixed bottom-0 right-0 z-50 transition-all duration-300 ${isOpen ? 'w-full md:w-96 h-[600px] mb-0 mr-0' : 'w-16 h-16 mb-20 mr-4 md:mb-6 md:mr-6'}`}>
            {/* Toggle Button */}
            {!isOpen && (
                <button
            ...
            {/* Chat Window */}
            {isOpen && (
                <div className="flex flex-col h-full bg-gray-800 border-t border-l border-gray-700 rounded-tl-2xl shadow-2xl overflow-hidden md:rounded-tr-2xl md:border-r">
                    {/* Header */}
                    <div className="flex items-center justify-between p-4 bg-gray-900 border-b border-gray-700">
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                            <span className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600">MTG Forge</span>
                        </div>
                    ...
                    <form onSubmit={handleSend} className="p-4 bg-gray-900 border-t border-gray-700">
                        <div className="flex gap-2">
                            <input
                                ...
                                placeholder="Ask about rules, decks, or lore..."
