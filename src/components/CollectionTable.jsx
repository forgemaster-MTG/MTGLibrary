...
const CollectionTable = ({ cards, isMixed }) => {
    ...
    const HeaderCell = ({ label, sortKey, className = "" }) => (
        <th
            className={`px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider cursor-pointer group hover:text-white transition-colors select-none ${className}`}
            ...
            <table className="w-full min-w-[1000px] border-collapse">
                ...
                <tbody className="divide-y divide-gray-800/50">
                    {sortedCards.map((card) => (
                        <tr key={card.id || card.firestoreId} className="group hover:bg-gray-800/30 transition-colors">
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                    <div className="h-10 w-10 flex-shrink-0 relative rounded overflow-hidden mr-4 border border-gray-700 bg-gray-900 group-hover:scale-150 group-hover:z-10 transition-all origin-left">
                                        <img className="h-full w-full object-cover" src={card.image_uri || card.image_uris?.small} alt="" />
                                    </div>
                                    <div className="ml-0">
