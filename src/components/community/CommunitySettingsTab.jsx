    ...
    return (
        <div className="space-y-8 animate-fade-in relative z-0">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-gray-800 pb-6 relative">
                <div>
                    <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-primary-400 to-purple-400 mb-2">My Pod</h2>
                ...
                <button
                    ...
                    className="flex items-center gap-2 text-primary-400 hover:text-white transition-colors text-sm font-bold bg-primary-900/20 hover:bg-primary-900/40 px-4 py-2 rounded-lg border border-primary-500/30"
                >
            ...
            {loading ? (
                ...
                <div className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-3xl border border-gray-800 p-8 md:p-12 text-center animate-fade-in-up shadow-2xl relative overflow-hidden">
                    ...
                    <div className="max-w-md mx-auto relative z-10">
                        <div className="w-20 h-20 bg-primary-500/10 text-primary-400 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-xl ring-1 ring-inset ring-primary-500/20">
                        ...
                        <form onSubmit={handleSendRequest} className="space-y-6">
                            <div className="flex p-1 bg-black/40 border border-gray-700 rounded-xl max-w-xs mx-auto">
                                <button
                                    ...
                                    className={`flex-1 py-2 px-4 rounded-lg text-sm font-bold transition-all ${addType === 'pod' ? 'bg-primary-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
                                >
                                    Pod Invite
                                </button>
                                <button
                            ...
                            <div className="space-y-4">
                                <input
                                    ...
                                    className="w-full bg-black/40 border border-gray-700 text-white px-5 py-4 rounded-xl focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-all text-center placeholder-gray-600 shadow-inner"
                                />
                                <button
                                    type="submit"
                                    disabled={addLoading}
                                    className="w-full bg-gradient-to-r from-primary-600 to-purple-600 hover:from-primary-500 hover:to-purple-500 text-white font-bold py-4 px-6 rounded-xl transition-all shadow-lg shadow-primary-900/20 transform hover:-translate-y-1"
                                >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-gray-900/30 rounded-3xl border border-gray-800 p-8 shadow-xl">
                            <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-3">
                                <span className="bg-gradient-to-r from-primary-500 to-purple-500 text-white p-2 rounded-lg shadow-lg">
                            ...
                            {friends.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {friends.map(friend => (
                                        <div key={friend.id} className="relative group bg-gray-800/40 p-5 rounded-2xl border border-gray-700/50 hover:border-primary-500/50 transition-all duration-300 hover:bg-gray-800/60 hover:shadow-xl hover:shadow-primary-900/10">
                                            <div className="flex items-center gap-4 mb-4">
                                                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary-600 to-purple-700 flex items-center justify-center text-white font-black text-2xl shadow-lg ring-2 ring-gray-900">
                                                    {friend.friend.username?.[0]?.toUpperCase() || '?'}
                                                </div>
                                                <div className="min-w-0">
                                                    <div className="font-bold text-white text-lg truncate group-hover:text-primary-300 transition-colors uppercase">{friend.friend.username}</div>
                                                    <div className="flex items-center gap-2">
                                                        <span className={`text-[10px] uppercase tracking-widest font-black px-2 py-0.5 rounded ${friend.type === 'friend' ? 'bg-purple-900/30 text-purple-400 border border-purple-800/50' : 'bg-primary-900/30 text-primary-400 border border-primary-800/50'}`}>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-gray-900/30 rounded-2xl border border-gray-800 p-6">
                            ...
                            <div className="flex p-1 bg-black/40 border border-gray-700 rounded-xl mb-4">
                                <button
                                    type="button"
                                    onClick={() => setAddType('pod')}
                                    className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition-all ${addType === 'pod' ? 'bg-primary-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
                                >
                                    Pod
                                </button>
                                <button
                            ...
                            <form onSubmit={handleSendRequest} className="space-y-3">
                                <input
                                    ...
                                    value={addEmail}
                                    onChange={(e) => setAddEmail(e.target.value)}
                                    className="w-full bg-black/40 border border-gray-700 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-primary-500 text-sm transition-all"
                                />
                                <button
                                    type="submit"
                                    disabled={addLoading}
                                    className={`w-full ${addType === 'friend' ? 'bg-purple-600 hover:bg-purple-500' : 'bg-primary-600 hover:bg-primary-500'} text-white font-bold py-3 px-4 rounded-xl transition-all shadow-lg text-sm flex items-center justify-center gap-2`}
                                >
                        ...
                        <div className="bg-gradient-to-br from-primary-900/10 to-purple-900/10 rounded-2xl border border-primary-500/20 p-6 space-y-4">
                            <h3 className="text-sm font-black text-transparent bg-clip-text bg-gradient-to-r from-primary-400 to-purple-400 uppercase tracking-wider mb-2">Referral & Sharing</h3>
                            ...
                            <div className="space-y-2">
                                <div className="bg-black/40 border border-gray-700/50 rounded-xl p-3 flex items-center gap-2">
                                    <span className="text-[10px] text-primary-400 font-mono truncate flex-1 leading-none select-all pt-1">https://mtg-forge.com/?ref=${userProfile?.username || user.uid}</span>
                                    <button
                                        onClick={() => {
                                            navigator.clipboard.writeText(`https://mtg-forge.com/?ref=${userProfile?.username || user.uid}`);
                                            alert('Referral link copied!');
                                        }}
                                        className="p-1.5 hover:bg-primary-500/20 text-primary-400 rounded-lg transition-colors"
                                    >
                        ...
                        {incomingRequests.length > 0 && (
                            <div className="bg-gradient-to-br from-primary-900/30 to-purple-900/30 rounded-2xl border border-primary-500/30 p-6 animate-pulse-border">
                                ...
                                <div className="space-y-3">
                                    {incomingRequests.map(req => (
                                        <div key={req.id} className="bg-gray-900/80 p-3 rounded-xl border border-primary-500/20 shadow-lg">
                                            <div className="flex items-center gap-3 mb-3">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="text-xs text-primary-300 truncate">wants to join your Pod</div>
                                                </div>
                                            </div>
                                            <div className="flex gap-2">
                                                <button
                                                    onClick={() => handleRespond(req.id, 'accepted')}
                                                    className="flex-1 bg-primary-500 hover:bg-primary-400 text-white text-xs font-bold py-2 rounded-lg transition-colors shadow-md"
                                                >
                                                    Accept
                                                </button>
                                                <button
