...
const CommunitySettingsTab = () => {
    ...
    const handleSendRequest = async (e) => {
        ...
        setAddMessage(null);
        try {
            await communityService.sendRequest(addEmail);
            setAddMessage({ type: 'success', text: 'Request sent!' });
            setAddEmail('');
            fetchData();
        } catch (err) {
                                        ...
                                        <div key={friend.id} className="relative group bg-gray-800/40 p-5 rounded-2xl border border-gray-700/50 hover:border-indigo-500/50 transition-all duration-300 hover:bg-gray-800/60 hover:shadow-xl hover:shadow-indigo-900/10">
                                            <div className="flex items-center gap-4 mb-4">
                                                ...
                                                <div className="min-w-0">
                                                    <div className="font-bold text-white text-lg truncate group-hover:text-indigo-300 transition-colors">{friend.friend.username}</div>
                                                    <div className="text-xs text-gray-500 uppercase tracking-widest font-bold">Viewer Access</div>
                                                </div>
                                            </div>
                        ...
                        <div className="bg-gray-900/30 rounded-2xl border border-gray-800 p-6">
                            ...
                            <form onSubmit={handleSendRequest} className="space-y-3">
                                ...
                                <button
                                    type="submit"
                                    disabled={addLoading}
                                    className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 px-4 rounded-xl transition-all shadow-lg text-sm flex items-center justify-center gap-2"
                                >
                                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                                    {addLoading ? 'Sending...' : 'Invite Friend'}
                                </button>
                                {addMessage && (
